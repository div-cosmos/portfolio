...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    if (state.phase === 'typing') {
      if (state.text === role) {
        t = setTimeout(() => setState(s => ({ ...s, phase: 'pausing' })), 2000)
      } else {
        t = setTimeout(() => setState(s => ({ ...s, text: role.slice(0, s.text.length + 1) })), 88)
      }
    } else if (state.phase === 'pausing') {
      t = setTimeout(() => setState(s => ({ ...s, phase: 'erasing' })), 100)
    } else {
      if (state.text === '') {
        setState(s => ({ text: '', idx: (s.idx + 1) % ROLES.length, phase: 'typing' }))
      } else {
        t = setTimeout(() => setState(s => ({ ...s, text: s.text.slice(0, -1) })), 46)
      }
    }
    ...
    <section className="hero">
      ...
      <div className="container hero__body">
        ...
        <p className="hero__desc">
          ...
          patterns. Specializing in React, TypeScript, and micro-frontend systems that
        ...
        <div className="hero__socials">
          ...
          <a href="mailto:divyanshuverma919@gmail.com" className="hero__social" title="Email">
            <MailIcon />
          </a>
          <a href="https://linkedin.com/in/divyanshu-verma" target="_blank" rel="noopener noreferrer" className="hero__social" title="LinkedIn">
            <LinkedInIcon />
          </a>
